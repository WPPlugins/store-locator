...
function sl_head_scripts() {
	...
	if (isset($GLOBALS['is_on_sl_page']) && $GLOBALS['is_on_sl_page'] == 1) {
		...
		if (empty($_POST) && 1==1) { //skip, for now always (1==2), dynamic file always causes trouble for some //1==1 - v3.98
			...
			print "<script src='".SL_JS_BASE."/store-locator-js.php?v=".SL_VERSION."'  type='text/javascript'></script>\n";
		} else {
...
function sl_add_options_page() {
  ...
  //   }	
	...
	if ( defined("SL_ADDONS_PLATFORM_DIR") ) {
	   ...
	   if (version_compare($ap_latest_version, $ap_version) > 0) {
...
function sl_add_admin_javascript() {
		...
		wp_enqueue_script( 'sl_func', SL_JS_BASE."/store-locator-js.php?admin=1", "jQuery");
...
function js_out($buff) {
	...
	$the_js[0]=($_GET['debug']!=1)? preg_replace("@\r|\n|\t|[[:space:]]{2,}@s","",$the_js[0]) : $the_js[0];
